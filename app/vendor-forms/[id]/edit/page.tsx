...
import { mockVendors, mockVendorForms } from '@/_mocks/vendor-forms';
...
export default function VendorFormEditPage({ params }: Props) {
  const router = useRouter();
  const { id } = use(params);
  const [selectedVendor, setSelectedVendor] = useState<IVendor | null>(null);
  const [form, setForm] = useState<Omit<IVendorForm, 'id' | 'vendorName' | 'vendorCode' | 'ceo' | 'createdBy' | 'createdAt' | 'updatedBy' | 'updatedAt'>>({
    invoiceItems: {
      paymentInfo: true,
      membership: true,
      barcode: false,
  ...
  useEffect(() => {
    // 목업 데이터에서 해당 ID의 양식 찾기
    const vendorForm = mockVendorForms.find(form => form.id === Number(id));
    if (vendorForm) {
      // 해당 사업자 정보 찾기
      const vendor = mockVendors.find(v => v.code === vendorForm.vendorCode);
      if (vendor) {
        setSelectedVendor(vendor);
      }
      // 양식 정보 설정
      setForm({
        invoiceItems: vendorForm.invoiceItems,
        callCenterInfo: vendorForm.callCenterInfo || '',
        paymentDescription: vendorForm.paymentDescription || '',
      });
    } else {
      // 찾지 못한 경우 목록으로 이동
      toast.error('존재하지 않는 청구서 양식입니다.');
        ...
        <div className="max-w-7xl mx-auto">
          <div className="container mx-auto py-6">
            <div className="flex justify-between items-center mb-6">
              <h1 className="text-2xl font-bold">청구서 양식 수정</h1>
